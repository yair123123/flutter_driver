  ...
  FormDispatchNotifier(this.ref)
    : super(
        DispatchState.initial(
          ...
          ref.read(initialScreenProvider).value!,
        ),
      );

  void selectTemplate(String selectedValue) {
    
    final text = state.rideDetails;
    final lines = text.split('\n');
    while (lines.length <= state.indexCurrentLine) {
      lines.add('');
    }
    
    lines[state.indexCurrentLine] = selectedValue;
    final newText = lines.join('\n');
    state = state.copyWith(rideDetails: newText);
  }
